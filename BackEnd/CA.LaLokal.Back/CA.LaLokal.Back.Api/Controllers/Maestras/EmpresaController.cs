...
namespace CA.LaLokal.Back.Api.Controllers.Maestras
{
    ...
    public class EmpresaController : ApiControllerBase
    {
        ...
        {
            var result = await Mediator.Send(new GetEmpresaQuery() { Id = id });
        ...
        [HttpPost]
        public async Task<ActionResult<int>> Create(CreateEmpresaCommand command)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<ActionResult<bool>> Update(int id, [FromBody] UpdateEmpresaCommand command)
        {
            if (id != command.Id)
            {
                return BadRequest();
